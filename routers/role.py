...
from helpers.exceptions import http_exception_not_found, http_exception_unauthorized
...
async def get_role(role_id: int, db: Annotated[Session, Depends(get_db)]):
...
async def get_role_by_name(role_type: str, db: Annotated[Session, Depends(get_db)]):
...
async def create_role(request: RoleRequest, db: Annotated[Session, Depends(get_db)]):
...
async def update_role(role_id: int, request: RoleRequest, db: Annotated[Session, Depends(get_db)]):
    db_role = db.query(Role).filter(Role.id == role_id).first()
    if not db_role:
        raise http_exception_not_found(f"Role with id {role_id} not found")
...
async def delete_role(role_id: int, db: Annotated[Session, Depends(get_db)]):
    db_role = db.query(Role).filter(Role.id == role_id).first()
    if not db_role:
        raise http_exception_not_found(f"Role with id {role_id} not found")
